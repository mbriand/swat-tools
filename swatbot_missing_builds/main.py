...
def find(buildbot_url, buildid_min, buildid_max, output: TextIO):
    ...
    def process_buildid(buildid):
        try:
            status = buildbot_operations.check_build_is_missing(
                base_url, rest_url, buildid
            )
            ...
            logger.warning("Build %s not found on buildbot server", buildid)
        except (
            ...
            logger.exception("Failed to analyze build %s: %s", buildid, err)
        ...
        for buildid in progress:
            try:
                if buildid in input_data["create_builds"]:
                    add_success = swatbot_operations.add_build(
                        rest_url, buildbot_url, buildid, dry_run
                    )
                    if not add_success:
                        continue
                swatbot_operations.update_build(
                    rest_url, buildbot_url, buildid, dry_run
                )
