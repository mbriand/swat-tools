...
class _Filter:
    ...
    def match(self, line: str) -> tuple[bool, Optional[_Highlight], list[int]]:
        ...
        hilight = _Highlight(
            ...
            self.color,
            text,
            in_logview=self.in_logview,
            in_menu=self.in_menu,
            in_bugzilla=self.in_bugzilla,
        )
        context = list(range(-self.context_before, 0)) + list(
            range(1, self.context_after + 1)
        )
...
class Log:
    ...
    _cached_log_highlights: dict[
        tuple[swatbuild.Failure, str], dict[int, _Highlight]
    ] = {}
    ...
    def _get_log_highlights_filters(self, loglen: int) -> list[_Filter]:
        ...
        if loglen > BIG_LOG_LIMIT:
            logging.warning(
                "Log file for build %s (failure %s) is quite big: "
                "using simplified log filters",
                self.failure.build.id,
                self.failure.id,
            )
            filters = [
                _Filter(
                    re.compile(r"(?P<keyword>\S*error):", flags=re.I),
                    utils.Color.RED,
                    in_menu=is_error,
                ),
                _Filter(
                    re.compile(r"(?P<keyword>\S*warning):", flags=re.I),
                    utils.Color.YELLOW,
                    in_menu=is_warning,
                ),
            ]
            ...
            filters = [
                ...
                _Filter(
                    re.compile(r".*except\s*\S*error:", flags=re.I),
                    enabled=test == "toaster",
                    in_logview=False,
                ),
                _Filter(
                    re.compile(
                ...
                _Filter(
                    re.compile(r".*test_fixed_size_error:"), in_logview=False
                ),
                _Filter(re.compile(r".*( |::)error::.*ok"), in_logview=False),
                _Filter(
                    re.compile(r"(.*\s|^)(?P<keyword>\S*error):", flags=re.I),
                    utils.Color.RED,
                    in_menu=is_error,
                ),
                _Filter(
                    re.compile(
                        r"(.*\s|^)(?P<keyword>\S*warning):", flags=re.I
                    ),
                    utils.Color.YELLOW,
                    in_menu=is_warning,
                ),
                _Filter(
                    re.compile(r"^(?P<keyword>fatal):", flags=re.I),
                    utils.Color.RED,
                    in_menu=is_error,
                ),
                _Filter(
                    re.compile(r"(.*\s|^)(?P<keyword>make\[\d\]):.* Error"),
                    utils.Color.RED,
                    in_menu=is_error,
                ),
                _Filter(
                ...
                _Filter(
                    re.compile(r"^(?P<keyword>command timed out:)"),
                    utils.Color.RED,
                    in_menu=True,
                ),
                _Filter(
                    re.compile(r".* \.\.\. (?P<keyword>FAIL[^ ]*).*"),
                    utils.Color.RED,
                    in_menu=True,
                ),
                _Filter(
                    re.compile(r"(?P<keyword>Failed ptests:)"),
                    utils.Color.RED,
                    in_menu=True,
                    context_after=1,
                ),
    ...
    @staticmethod
    def _build_log_highlights(
        loglines: list[str], filters: list[_Filter]
    ) -> dict[int, _Highlight]:
        highlight_lines = {}
    ...
    def _load_cache_file(
        self, num_lines: int, filtershash: str
        ...
        with gzip.open(cachefile, mode="r") as file:
            try:
                ...
                if (
                    data["version"] == HILIGHTS_FORMAT_VERSION
    ...
    def _load_log_highlights(self):
        ...
        self._highlights = self._load_cache_file(
            logmetadata["num_lines"], filtershash.hexdigest()
        )
